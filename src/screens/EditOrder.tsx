...
const EditOrder: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const exists = await checkOrderExists(formData.orderNumber, orderId);
      if (exists) {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-4">
        <Link to={`/orders/${orderId}`} className="p-2 bg-white rounded-xl shadow-sm border border-gray-100 hover:bg-gray-50 transition-colors">
          <ChevronLeft size={24} className="text-gray-600" />
        </Link>
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Order</h1>
          <p className="text-gray-500">Modify order details.</p>
        </div>
      </div>
